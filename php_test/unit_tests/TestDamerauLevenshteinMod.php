<?php
require_once(dirname(__FILE__) . "/../../php/DamerauLevenshteinMod.php");
...
class TestDamerauLevenshteinMod extends UnitTestCase
{
	...
	{
		$file = file(dirname(__FILE__) . "/../../spec/test.txt");
		foreach($file as $line => $test_case)
		{
			if(!preg_match("/^\s*#/", $test_case) && preg_match("/^([^\|]+)\|([^\|]+)\|([^\|]+)\|([^\|]+)\|([^\|]+)$/", $test_case, $arr))
			{
				...
				echo $res . "/$test_result ### $test_value_1, $test_value_2, $test_block_size, $test_max_distance\n";
